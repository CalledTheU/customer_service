...
# 查询流程状态action
class QueryFlowStatus(Action):
    # name值和yaml文件查询流程状态流程action属性值相同
    name = "action_query_flow_status"
    ...
    async def run(self, state: DialogueState) -> ActionResult:
        ...
        # 拼接协同平台查询流程状态接口地址
        url = f"{settings.commerce_api_base_url}/flows/{flow_number}/status"
        ...
        return ActionResult(
            slot_updates={
                "flow_status": data.get("status", "未知"),
                "flow_summary": data.get("status_desc", "未知")
            }
        )
